...
def load_low_rank_model(init_model,experiment,model_type,custom_objects = None):
    new_model = keras.models.clone_model(init_model)
    tt_prior = get_tt_prior(new_model)
    new_model.load_weights('./saved_models/'+experiment+'/'+model_type+'_weights.h5')
    return new_model,tt_prior

def count_removed_params(model,sess,threshold = 1e-3):
    removed_params = 0
...
def estimate_rank_from_prior_variances(prior_variances,threshold= 1e-3):
...
def get_svgd_update(model_list, priors_list, batch_size,svgd_stepsize,sess,svgd_subsampling_ratio=None,constant_subsample=True):
    ...
    sess.run(tf.variables_initializer([historical_grad]))
    
    kernel_matrix, kernel_derivative = svgd_kernel(unpacked_model_weights,svgd_subsampling_ratio,constant_subsample)
    ...
    return historical_grad_assign_op,particle_update_ops,input_output_placeholders
...
#        with tf.device(string):
        ...
        model = model_list[i]
        temp = get_placeholders_and_gradients(model,batch_size,model.output_shape[1])
        ...
        unpacked_model_weights_list.append(tf_model_unpack(model))
        
    return model_gradients_list, input_output_placeholders,unpacked_model_gradients_list,unpacked_model_weights_list


def get_cifar_100_conv_tt_prior(model, invert_param = False,scale = 100):
    ...
    for layer in model.layers:
        if hasattr(layer,'tt_rank'):
            temp_prior += (1e-4)*layer.create_layer_prior(bias_variance = scale,invert=invert_param)
        else:
            for weight in layer.trainable_weights:
                temp_prior+= (1e-4)*tf.reduce_sum(dist.log_prob(weight))
    return temp_prior
...
def make_particles(first_model, num_particles, optimizer,x_train,y_train,batch_size,datagen=None,x_test = None, y_test = None, verbose = 0, prior = 'tt',steps_per_epoch = None):
    ...
    for i in range(0,num_particles):
        ...
        temp_model.set_weights(first_model.get_weights())
        ...
        if datagen is not None:
            temp_model.fit_generator(datagen.flow(x_train, y_train,
                                         batch_size=batch_size),
                            steps_per_epoch= steps_per_epoch,
                            epochs=1,
                            workers=4)
        else:        
                temp_model.fit(x_train, y_train, epochs=1, batch_size=batch_size, validation_data=(x_test, y_test),verbose=verbose)
        
        model_list.append(temp_model)   
    
    return model_list, priors_list
...
def svgd_kernel(X0,subsampling_ratio=None,constant_subsample=True):

    #if we subsample, generate the columns to sample
    full = X0
    if subsampling_ratio and subsampling_ratio<1.0 and subsampling_ratio>0.0:

        d = X0.shape.as_list()[1]
        sample_size = int(subsampling_ratio*d)
        rescale = d/sample_size
        
        if constant_subsample:
            idx = tf.constant(np.random.choice(d,size=sample_size,replace=False))
        else:
            idx = tf.random_shuffle(tf.range(d))[:sample_size]        
        
        X0 = tf.gather(X0,idx,axis=1)     
    ...
    dxkxy = -tf.matmul(Kxy, full)
    sumkxy = tf.expand_dims(tf.reduce_sum(Kxy, axis=1), 1)
    dxkxy = tf.add(dxkxy, tf.multiply(full, sumkxy)) / (h ** 2)
