...
class reshapedBayesKerasConv(Layer):
    ...
    def __init__(self, input_channels, input_channel_modes, output_channels, 
               ...
               kernel_initializer='cvpr_2018',bias_initializer=0.0, low_rank_init = None, **kwargs):
...
#        b_lambda = 5.*tf.ones([1],dtype=tf_precision)
        if invert:
            a_lambda = 1e-6*tf.ones([1],dtype=tf_precision)
            b_lambda = 1e-6*tf.ones([1],dtype=tf_precision)
        else:
            a_lambda = 1.*tf.ones([1],dtype=tf_precision)
            b_lambda = 10.*tf.ones([1],dtype=tf_precision)
        ...
        core_priors = []
        def new_tt_core_prior(left_core,right_core,prior_variance, last_core = False):
           # left_core_shape = left_core.shape.as_list()
           # right_core_shape = right_core.shape.as_list()
    
            prior_normal = tfd.MultivariateNormalDiag(loc = tf.zeros(shape = [self.tt_rank],dtype = tf_precision),scale_diag = prior_variance)
            
            if last_core:                
#                return tf.reduce_sum(prior_normal.log_prob(tf.transpose(left_core)))
                return 0.0#tf.reduce_sum(prior_normal.log_prob(tf.transpose(left_core,perm = [2,1,0])))
            else:
#                return tf.reduce_sum(prior_normal.log_prob(tf.transpose(left_core,perm = [3,0,1,2])))
                return tf.reduce_sum(prior_normal.log_prob(left_core))

        for i in range(len(self.tt_shape)+1):
           # print(i)
           # print(self.trainable_weights[i])
            if i==(len(self.tt_shape)):
                #normal prior on bias
                core_priors.append(tf.reduce_sum(tfd.Normal(loc = 0,scale = bias_variance).log_prob(self.trainable_weights[i])))
            elif i==(len(self.tt_shape)-1):
                #last core
                core_priors.append(new_tt_core_prior(self.trainable_weights[i],[],self.prior_variances[i-1],last_core = True))    
            else:
                core_priors.append(new_tt_core_prior(self.trainable_weights[i],self.trainable_weights[i+1],self.prior_variances[i]))    
        
        prior = tf.reduce_sum(core_priors+[local_prior])
